...
Le profil d'index sémantique est passé à `content-v4`. Un index créé avec l'ancien profil n'est donc pas considéré compatible : la prochaine indexation reconstruit les vecteurs afin de ne pas conserver silencieusement des embeddings historiques issus d'un contenu non redigé.
